...
function ConversationItem({ conv, selected, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left px-4 py-3 border-b border-gray-100 hover:bg-gray-50 transition-colors flex gap-3 ${selected ? 'bg-indigo-50/70 border-l-2 border-l-indigo-500' : ''}`}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1 mt-1.5 flex-wrap">
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500">{conv.channel?.name}</span>
          <span className={`text-[10px] px-1.5 py-0.5 rounded-full border ${stage.color}`}>{stage.label}</span>
          {conv.agent && <span className="text-[10px] text-indigo-600">→ {conv.agent.name}</span>}
...
function MessageBubble({ msg }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-start' : 'justify-end'} mb-2`}>
      <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl text-sm shadow-sm ${isUser ? 'bg-white border border-gray-100 text-gray-800 rounded-tl-sm' : 'bg-gradient-to-br from-green-500 to-emerald-600 text-white rounded-tr-sm'}`}>
...
function FilterPanel({ filter, setFilter, channels, agents, tags, onClose }) {
  return (
    <div className="absolute top-full left-3 right-3 mt-1 bg-white border rounded-xl shadow-lg z-20 p-3 space-y-3">
      <div>
        <p className="text-xs font-medium text-gray-500 mb-1.5">สถานะ</p>
        <div className="flex gap-1 flex-wrap">
          {['', 'open', 'pending', 'closed'].map(s => (
            <button
              key={s || 'all'}
              onClick={() => setFilter(f => ({ ...f, status: s }))}
              className={`text-xs px-2 py-1 rounded-full border transition-colors ${filter.status === s ? 'bg-indigo-500 text-white border-indigo-500' : 'text-gray-600 hover:border-gray-400'}`}
            >
      ...
      <div>
        <p className="text-xs font-medium text-gray-500 mb-1.5">Lifecycle</p>
        <div className="flex gap-1 flex-wrap">
          <button
            onClick={() => setFilter(f => ({ ...f, lifecycleStage: '' }))}
            className={`text-xs px-2 py-1 rounded-full border transition-colors ${filter.lifecycleStage === '' ? 'bg-indigo-500 text-white border-indigo-500' : 'text-gray-600 hover:border-gray-400'}`}
          >
            ทั้งหมด
          </button>
          {LIFECYCLE_STAGES.map(s => (
            <button
              ...
              className={`text-xs px-2 py-1 rounded-full border transition-colors ${filter.lifecycleStage === s.key ? s.color + ' font-medium' : 'text-gray-500 border-gray-200 hover:border-gray-400'}`}
            >
              {s.label}
            </button>
          ))}
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <p className="text-xs font-medium text-gray-500 mb-1.5">ช่องทาง</p>
          <select
        ...
        <div>
          <p className="text-xs font-medium text-gray-500 mb-1.5">พนักงาน</p>
          <select
            className="w-full text-xs border rounded-lg px-2 py-1.5 focus:outline-none"
      ...
      {tags.length > 0 && (
        <div>
          <p className="text-xs font-medium text-gray-500 mb-1.5">แท็ก</p>
          <div className="flex gap-1 flex-wrap">
            <button
              onClick={() => setFilter(f => ({ ...f, tagId: '' }))}
              className={`text-xs px-2 py-1 rounded-full border transition-colors ${filter.tagId === '' ? 'bg-indigo-500 text-white border-indigo-500' : 'text-gray-600 hover:border-gray-400'}`}
            >
              ทั้งหมด
            </button>
            {tags.map(t => (
      ...
      <button onClick={onClose} className="text-xs text-gray-400 hover:text-gray-600 w-full text-center pt-1">ปิด</button>
    </div>
  );
}
...
function CustomerPanel({ conv, tags, onUpdate, onAddTag, onRemoveTag, onCreateTag, onClose }) {
  ...
  return (
    <div className="w-72 bg-white border-l flex flex-col overflow-y-auto flex-shrink-0">
      <div className="p-4 border-b flex items-center justify-between">
        <h3 className="font-semibold text-gray-900 text-sm">รายละเอียดลูกค้า</h3>
        <button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X size={16} /></button>
      </div>

      <div className="p-4 flex flex-col items-center border-b">
        <Avatar name={conv.displayName} pictureUrl={conv.pictureUrl} size={16} />
        <p className="text-xs text-gray-400 mt-2">{conv.channel?.name}</p>
      </div>

      <div className="p-4 border-b">
        <label className="text-xs font-medium text-gray-500 mb-1 flex items-center gap-1"><Pencil size={11}/> ชื่อลูกค้า</label>
      ...
      <div className="p-4 border-b">
        <label className="text-xs font-medium text-gray-500 mb-1.5 block">Lifecycle stage</label>
        <select
      ...
      <div className="p-4 border-b">
        <div className="flex items-center justify-between mb-1.5">
          <label className="text-xs font-medium text-gray-500 flex items-center gap-1"><TagIcon size={11}/> แท็ก</label>
          <button onClick={() => setShowTagPicker(v => !v)} className="text-indigo-500 hover:text-indigo-700"><Plus size={14} /></button>
        </div>
        <div className="flex flex-wrap gap-1.5 mb-2">
          {(conv.tags || []).length === 0 && <p className="text-xs text-gray-300">ยังไม่มีแท็ก</p>}
          {(conv.tags || []).map(({ tag }) => (
          ...
          <div className="border rounded-lg p-2 bg-gray-50 space-y-1.5">
            {availableTags.map(t => (
              <button
                ...
                className="flex items-center gap-2 text-xs w-full text-left px-1.5 py-1 rounded hover:bg-white"
              >
            ...
            <div className="flex gap-1 pt-1">
              <input
                className="flex-1 text-xs border rounded px-1.5 py-1 focus:outline-none"
      ...
      <div className="p-4 flex-1">
        <label className="text-xs font-medium text-gray-500 mb-1.5 block">โน้ต</label>
        <textarea
          className="w-full border rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
          rows={6}
        ...
        <div className="p-3 border-b relative">
          ...
          <div className="relative mb-2 flex gap-1.5">
            <div className="relative flex-1">
              ...
              <input
                className="w-full pl-7 pr-3 py-1.5 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                placeholder="ค้นหา..."
                value={filter.search}
            ...
            <button
              ...
              className={`relative flex items-center justify-center w-9 h-9 rounded-lg border transition-colors ${showFilters || activeFilterCount ? 'bg-indigo-500 border-indigo-500 text-white' : 'text-gray-500 hover:border-gray-400'}`}
            >
        ...
        <>
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="bg-white border-b px-4 py-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-medium text-gray-900 truncate">{selected.displayName || selected.lineUserId}</h3>
              ...
              <button
                ...
                className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${showDetail ? 'bg-indigo-100 text-indigo-600' : 'text-gray-400 hover:bg-gray-100'}`}
